...
const char *freetds_conf_path = NULL;
const char *pool_conf_path = NULL;
...
tds_read_conf_file(TDSLOGIN * login, const char *server)
{
	...
	if (!found && freetds_conf_path) {
		found = tds_try_conf_file(freetds_conf_path, "(default)", server, login);
	}
