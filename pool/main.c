...
extern const char *freetds_conf_path;
extern const char *pool_conf_path;
...
{
	fprintf(stderr, "Usage:\t%s [-l <log file>] [-s <server configuration file>] [-p <pool configuration file>] <pool name>\n", progname);
}
...
#  define DAEMON_OPT ""
	...
	freetds_conf_path = NULL;
	pool_conf_path = NULL;

	while ((opt = getopt(argc, argv, "l:s:p:" DAEMON_OPT)) != -1) {
		switch (opt) {
			case 'l':
				logfile_name = optarg;
				break;
			case 's':
				freetds_conf_path = optarg;
				break;
			case 'p':
				pool_conf_path = optarg;
				break;
#ifndef _WIN32
			case 'd':
				daemonize = true;
	...
	if (optind >= argc || !freetds_conf_path || !pool_conf_path) {
		print_usage(argv[0]);
		return EXIT_FAILURE;
	}
